import React, { forwardRef } from 'react';

const PianoKeyboardDuotone = forwardRef(({ size = 24, color = 'currentColor', className, ...props }, ref) => (
  <svg
    ref={ref}
    xmlns="http://www.w3.org/2000/svg"
    width={size}
    height={size}
    viewBox="0 0 576 512"
    fill={color}
    className={className}
    {...props}
  >
    <path d="M576 224V416C576 433.625 561.625 448 544 448H32C14.375 448 0 433.625 0 416V224H64V384H512V224H576Z" className="stria-secondary" style={{ opacity: 0.4 }} />
    <path d="M576 96V224H448V304C448 312.875 440.875 320 432 320H400C391.125 320 384 312.875 384 304V224H320V304C320 312.875 312.875 320 304 320H272C263.125 320 256 312.875 256 304V224H192V304C192 312.875 184.875 320 176 320H144C135.125 320 128 312.875 128 304V224H0V96C0 78.375 14.375 64 32 64H544C561.625 64 576 78.375 576 96Z" />
  </svg>
));

PianoKeyboardDuotone.displayName = 'PianoKeyboardDuotone';

export default PianoKeyboardDuotone;
