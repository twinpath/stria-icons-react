import React, { forwardRef } from 'react';

const SidebarDuotone = forwardRef(({ size = 24, color = 'currentColor', className, ...props }, ref) => (
  <svg
    ref={ref}
    xmlns="http://www.w3.org/2000/svg"
    width={size}
    height={size}
    viewBox="0 0 512 512"
    fill={color}
    className={className}
    {...props}
  >
    <path d="M512 96V416C512 451.346 483.346 480 448 480H224V32H448C483.346 32 512 60.652 512 96Z" className="stria-secondary" style={{ opacity: 0.4 }} />
    <path d="M64 32C28.654 32 0 60.652 0 96V416C0 451.346 28.654 480 64 480H224V32H64ZM136 336H88C74.746 336 64 325.254 64 312C64 298.744 74.746 288 88 288H136C149.254 288 160 298.744 160 312C160 325.254 149.254 336 136 336ZM136 240H88C74.746 240 64 229.254 64 216C64 202.744 74.746 192 88 192H136C149.254 192 160 202.744 160 216C160 229.254 149.254 240 136 240ZM136 144H88C74.746 144 64 133.254 64 120C64 106.744 74.746 96 88 96H136C149.254 96 160 106.744 160 120C160 133.254 149.254 144 136 144Z" />
  </svg>
));

SidebarDuotone.displayName = 'SidebarDuotone';

export default SidebarDuotone;
