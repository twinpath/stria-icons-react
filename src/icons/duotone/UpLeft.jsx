import React, { forwardRef } from 'react';

const UpLeftDuotone = forwardRef(({ size = 24, color = 'currentColor', className, ...props }, ref) => (
  <svg
    ref={ref}
    xmlns="http://www.w3.org/2000/svg"
    width={size}
    height={size}
    viewBox="0 0 384 512"
    fill={color}
    className={className}
    {...props}
  >
    <path d="M359.119 377.374L313.864 422.628C301.367 435.124 281.108 435.124 268.61 422.628L139 293.016L229.508 202.504L359.119 332.116C371.617 344.616 371.615 364.874 359.119 377.374Z" className="stria-secondary" style={{ opacity: 0.4 }} />
    <path d="M70.625 360.898C61.477 370.047 47.709 372.785 35.756 367.836C23.789 362.879 16 351.223 16 338.27V111.996C16 103.16 19.58 95.16 25.369 89.371C31.16 83.582 39.158 80 47.998 80H274.271C287.221 80 298.877 87.789 303.838 99.754C308.787 111.707 306.047 125.477 296.898 134.625L70.625 360.898Z" />
  </svg>
));

UpLeftDuotone.displayName = 'UpLeftDuotone';

export default UpLeftDuotone;
