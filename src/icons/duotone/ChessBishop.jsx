import React, { forwardRef } from 'react';

const ChessBishopDuotone = forwardRef(({ size = 24, color = 'currentColor', className, ...props }, ref) => (
  <svg
    ref={ref}
    xmlns="http://www.w3.org/2000/svg"
    width={size}
    height={size}
    viewBox="0 0 320 512"
    fill={color}
    className={className}
    {...props}
  >
    <path d="M320 496C320 504.836 312.836 512 304 512H16C7.164 512 0 504.836 0 496C0 475.131 13.4 457.549 32 450.938V432C32 423.162 39.164 416 48 416H272C280.836 416 288 423.162 288 432V450.938C306.6 457.549 320 475.131 320 496Z" className="stria-secondary" style={{ opacity: 0.4 }} />
    <path d="M312 287.875C312 339.5 289.875 361.75 256 372.5V416H64V372.5C30.125 361.75 8 339.5 8 287.875C8 214.5 67.75 106.75 120.625 62.5C106.625 59.125 96 47 96 32C96 14.375 110.375 0 128 0H192C209.625 0 224 14.375 224 32C224 47 213.375 59.125 199.375 62.5C223.75 83 249.5 117.25 270.25 155.125L162.375 263C159.25 266.125 159.25 271.25 162.375 274.375L173.75 285.625C176.875 288.75 181.875 288.75 185 285.625L285.25 185.375C301.25 220.75 312 257.25 312 287.875Z" />
  </svg>
));

ChessBishopDuotone.displayName = 'ChessBishopDuotone';

export default ChessBishopDuotone;
