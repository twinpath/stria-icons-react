import React, { forwardRef } from 'react';

const RouteInterstateDuotone = forwardRef(({ size = 24, color = 'currentColor', className, ...props }, ref) => (
  <svg
    ref={ref}
    xmlns="http://www.w3.org/2000/svg"
    width={size}
    height={size}
    viewBox="0 0 512 512"
    fill={color}
    className={className}
    {...props}
  >
    <path d="M17 192H494.781C486.232 307.73 426.361 441.764 255.891 512C85.42 441.764 25.549 307.73 17 192Z" className="stria-secondary" style={{ opacity: 0.4 }} />
    <path d="M494.891 192H17.109C13.29 140.279 19.667 92.189 31.095 55.125C34.721 43.5 46.097 36 57.35 39.125C75.603 44.25 95.104 47.125 115.482 47.125C164.614 47.125 209.119 31.125 241.623 5C245.874 1.625 250.874 0 256 0S266.126 1.625 270.377 5C303.006 31.125 347.386 47.125 396.518 47.125C416.896 47.125 436.397 44.25 454.65 39.25C465.903 36 477.279 43.5 480.905 55.125C492.333 92.189 498.71 140.279 494.891 192Z" />
  </svg>
));

RouteInterstateDuotone.displayName = 'RouteInterstateDuotone';

export default RouteInterstateDuotone;
