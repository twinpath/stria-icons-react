import React, { forwardRef } from 'react';

const AirFreshenerDuotone = forwardRef(({ size = 24, color = 'currentColor', className, ...props }, ref) => (
  <svg
    ref={ref}
    xmlns="http://www.w3.org/2000/svg"
    width={size}
    height={size}
    viewBox="0 0 384 512"
    fill={color}
    className={className}
    {...props}
  >
    <path d="M304 384H80C53.49 384 32 405.49 32 432V464C32 490.51 53.49 512 80 512H304C330.51 512 352 490.51 352 464V432C352 405.49 330.51 384 304 384ZM304 464H80V432H304V464Z" className="stria-secondary" style={{ opacity: 0.4 }} />
    <path d="M377.174 306.141L305.887 224H342.172C364.196 224 376.077 198.48 361.743 181.963L211.539 8.891C201.252 -2.965 182.684 -2.963 172.397 8.895L22.254 181.967C7.924 198.484 19.805 224 41.827 224H78.084L6.823 306.145C-8.81 324.164 4.151 352 28.174 352H160.006V384H224.006V352H355.825C379.85 352 392.811 324.16 377.174 306.141ZM192.006 104C178.752 104 168.006 93.254 168.006 80S178.752 56 192.006 56S216.006 66.746 216.006 80S205.26 104 192.006 104Z" />
  </svg>
));

AirFreshenerDuotone.displayName = 'AirFreshenerDuotone';

export default AirFreshenerDuotone;
