import { jsx } from "react/jsx-runtime";
import React, { forwardRef } from "react";
const DiagramStankeySolid = forwardRef(({ size = 24, color = "currentColor", className, ...props }, ref) => /* @__PURE__ */ jsx(
  "svg",
  {
    ref,
    xmlns: "http://www.w3.org/2000/svg",
    width: size,
    height: size,
    viewBox: "0 0 640 512",
    fill: color,
    className,
    ...props,
    children: /* @__PURE__ */ jsx("path", { d: "M404.234 160L346.635 256H592C600.836 256 608 263.164 608 272V304C608 312.836 600.836 320 592 320H276.771L315.172 384H576C593.674 384 608 398.326 608 416V448C608 465.674 593.674 480 576 480H297.064C274.584 480 253.752 468.205 242.186 448.928L164.828 320H64C46.326 320 32 305.674 32 288V224C32 206.326 46.326 192 64 192H235.766L313.123 63.072C324.689 43.795 345.521 32 368.002 32H576C593.674 32 608 46.326 608 64V128C608 145.674 593.674 160 576 160H404.234Z" })
  }
));
DiagramStankeySolid.displayName = "DiagramStankeySolid";
var DiagramStankey_default = DiagramStankeySolid;
export {
  DiagramStankey_default as default
};
