import { jsx } from "react/jsx-runtime";
import React, { forwardRef } from "react";
const CowbellSolid = forwardRef(({ size = 24, color = "currentColor", className, ...props }, ref) => /* @__PURE__ */ jsx(
  "svg",
  {
    ref,
    xmlns: "http://www.w3.org/2000/svg",
    width: size,
    height: size,
    viewBox: "0 0 448 512",
    fill: color,
    className,
    ...props,
    children: /* @__PURE__ */ jsx("path", { d: "M384.258 122.375C381.508 107.125 368.132 96 352.631 96H336.63V48C336.63 21.5 315.003 0 288.501 0H160.242C133.74 0 112.239 21.5 112.239 48V96H95.363C79.861 96 66.485 107.125 63.735 122.375L0.481 474.375C-2.849 493.33 11.49 512 32.108 512H415.885C436.534 512 450.84 493.312 447.513 474.375L384.258 122.375ZM160.008 48H288.017V96H160.008V48Z" })
  }
));
CowbellSolid.displayName = "CowbellSolid";
var Cowbell_default = CowbellSolid;
export {
  Cowbell_default as default
};
