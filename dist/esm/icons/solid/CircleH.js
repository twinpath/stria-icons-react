import { jsx } from "react/jsx-runtime";
import React, { forwardRef } from "react";
const CircleHSolid = forwardRef(({ size = 24, color = "currentColor", className, ...props }, ref) => /* @__PURE__ */ jsx(
  "svg",
  {
    ref,
    xmlns: "http://www.w3.org/2000/svg",
    width: size,
    height: size,
    viewBox: "0 0 512 512",
    fill: color,
    className,
    ...props,
    children: /* @__PURE__ */ jsx("path", { d: "M256 16C123.451 16 16 123.451 16 256S123.451 496 256 496S496 388.549 496 256S388.549 16 256 16ZM368 360C368 373.25 357.25 384 344 384S320 373.25 320 360V280H192V360C192 373.25 181.25 384 168 384S144 373.25 144 360V152C144 138.75 154.75 128 168 128S192 138.75 192 152V232H320V152C320 138.75 330.75 128 344 128S368 138.75 368 152V360Z" })
  }
));
CircleHSolid.displayName = "CircleHSolid";
var CircleH_default = CircleHSolid;
export {
  CircleH_default as default
};
