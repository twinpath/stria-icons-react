import { jsx } from "react/jsx-runtime";
import React, { forwardRef } from "react";
const PrescriptionBottleMedicalSolid = forwardRef(({ size = 24, color = "currentColor", className, ...props }, ref) => /* @__PURE__ */ jsx(
  "svg",
  {
    ref,
    xmlns: "http://www.w3.org/2000/svg",
    width: size,
    height: size,
    viewBox: "0 0 384 512",
    fill: color,
    className,
    ...props,
    children: /* @__PURE__ */ jsx("path", { d: "M32 448C32 483.199 60.801 512 96 512H288C323.199 512 352 483.199 352 448V128H32V448ZM96 304C96 295.199 103.199 288 112 288H160V240C160 231.199 167.199 224 176 224H208C216.801 224 224 231.199 224 240V288H272C280.801 288 288 295.199 288 304V336C288 344.799 280.801 352 272 352H224V400C224 408.799 216.801 416 208 416H176C167.199 416 160 408.799 160 400V352H112C103.199 352 96 344.799 96 336V304ZM360 0H24C10.75 0 0 10.75 0 24V72C0 85.25 10.75 96 24 96H360C373.25 96 384 85.25 384 72V24C384 10.75 373.25 0 360 0Z" })
  }
));
PrescriptionBottleMedicalSolid.displayName = "PrescriptionBottleMedicalSolid";
var PrescriptionBottleMedical_default = PrescriptionBottleMedicalSolid;
export {
  PrescriptionBottleMedical_default as default
};
