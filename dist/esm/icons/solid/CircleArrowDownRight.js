import { jsx } from "react/jsx-runtime";
import React, { forwardRef } from "react";
const CircleArrowDownRightSolid = forwardRef(({ size = 24, color = "currentColor", className, ...props }, ref) => /* @__PURE__ */ jsx(
  "svg",
  {
    ref,
    xmlns: "http://www.w3.org/2000/svg",
    width: size,
    height: size,
    viewBox: "0 0 512 512",
    fill: color,
    className,
    ...props,
    children: /* @__PURE__ */ jsx("path", { d: "M256 16C123.422 16 16 123.422 16 256S123.422 496 256 496S496 388.578 496 256S388.578 16 256 16ZM376 344C376 361.688 361.672 376 344 376H200C182.328 376 168 361.688 168 344S182.328 312 200 312H266.75L145.375 190.625C132.875 178.125 132.875 157.875 145.375 145.375S178.125 132.875 190.625 145.375L312 266.75V200C312 182.312 326.328 168 344 168S376 182.312 376 200V344Z" })
  }
));
CircleArrowDownRightSolid.displayName = "CircleArrowDownRightSolid";
var CircleArrowDownRight_default = CircleArrowDownRightSolid;
export {
  CircleArrowDownRight_default as default
};
