import { jsx } from "react/jsx-runtime";
import React, { forwardRef } from "react";
const SquareArrowDownLight = forwardRef(({ size = 24, color = "currentColor", className, ...props }, ref) => /* @__PURE__ */ jsx(
  "svg",
  {
    ref,
    xmlns: "http://www.w3.org/2000/svg",
    width: size,
    height: size,
    viewBox: "0 0 448 512",
    fill: color,
    className,
    ...props,
    children: /* @__PURE__ */ jsx("path", { d: "M384 32H64C28.654 32 0 60.654 0 96V416C0 451.346 28.654 480 64 480H384C419.346 480 448 451.346 448 416V96C448 60.654 419.346 32 384 32ZM416 416C416 433.645 401.645 448 384 448H64C46.355 448 32 433.645 32 416V96C32 78.355 46.355 64 64 64H384C401.645 64 416 78.355 416 96V416ZM308.688 260.688L240 329.375V144C240 135.156 232.844 128 224 128S208 135.156 208 144V329.375L139.312 260.688C133.062 254.438 122.937 254.438 116.688 260.688S110.438 277.063 116.688 283.312L212.688 379.312C215.812 382.438 219.906 384 224 384S232.188 382.438 235.312 379.312L331.312 283.312C337.562 277.062 337.562 266.937 331.312 260.688S314.938 254.438 308.688 260.688Z" })
  }
));
SquareArrowDownLight.displayName = "SquareArrowDownLight";
var SquareArrowDown_default = SquareArrowDownLight;
export {
  SquareArrowDown_default as default
};
