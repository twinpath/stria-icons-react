import { jsx } from "react/jsx-runtime";
import React, { forwardRef } from "react";
const ClapperboardLight = forwardRef(({ size = 24, color = "currentColor", className, ...props }, ref) => /* @__PURE__ */ jsx(
  "svg",
  {
    ref,
    xmlns: "http://www.w3.org/2000/svg",
    width: size,
    height: size,
    viewBox: "0 0 512 512",
    fill: color,
    className,
    ...props,
    children: /* @__PURE__ */ jsx("path", { d: "M448 32H64C28.654 32 0 60.654 0 96V416C0 451.346 28.654 480 64 480H448C483.346 480 512 451.346 512 416V96C512 60.654 483.346 32 448 32ZM433.375 64L337.375 160H230.625L326.625 64H433.375ZM281.375 64L185.375 160H78.625L174.625 64H281.375ZM32 96C32 78.355 46.355 64 64 64H129.375L33.375 160H32V96ZM480 416C480 433.645 465.645 448 448 448H64C46.355 448 32 433.645 32 416V192H480V416ZM480 160H382.625L469.797 72.828C476.008 78.668 480 86.822 480 96V160Z" })
  }
));
ClapperboardLight.displayName = "ClapperboardLight";
var Clapperboard_default = ClapperboardLight;
export {
  Clapperboard_default as default
};
