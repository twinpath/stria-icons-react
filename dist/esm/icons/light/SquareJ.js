import { jsx } from "react/jsx-runtime";
import React, { forwardRef } from "react";
const SquareJLight = forwardRef(({ size = 24, color = "currentColor", className, ...props }, ref) => /* @__PURE__ */ jsx(
  "svg",
  {
    ref,
    xmlns: "http://www.w3.org/2000/svg",
    width: size,
    height: size,
    viewBox: "0 0 448 512",
    fill: color,
    className,
    ...props,
    children: /* @__PURE__ */ jsx("path", { d: "M384 32H64C28.654 32 0 60.654 0 96V416C0 451.346 28.654 480 64 480H384C419.346 480 448 451.346 448 416V96C448 60.654 419.346 32 384 32ZM416 416C416 433.645 401.645 448 384 448H64C46.355 448 32 433.645 32 416V96C32 78.355 46.355 64 64 64H384C401.645 64 416 78.355 416 96V416ZM304 128C295.156 128 288 135.156 288 144V293.328C288 325.688 259.281 352 224 352S160 325.688 160 293.328V272C160 263.156 152.844 256 144 256S128 263.156 128 272V293.328C128 343.328 171.062 384 224 384S320 343.328 320 293.328V144C320 135.156 312.844 128 304 128Z" })
  }
));
SquareJLight.displayName = "SquareJLight";
var SquareJ_default = SquareJLight;
export {
  SquareJ_default as default
};
