import { jsx, jsxs } from "react/jsx-runtime";
import React, { forwardRef } from "react";
const DatabaseDuotone = forwardRef(({ size = 24, color = "currentColor", className, ...props }, ref) => /* @__PURE__ */ jsxs(
  "svg",
  {
    ref,
    xmlns: "http://www.w3.org/2000/svg",
    width: size,
    height: size,
    viewBox: "0 0 448 512",
    fill: color,
    className,
    ...props,
    children: [
      /* @__PURE__ */ jsx("path", { d: "M448 73.125V118.875C448 159.125 347.625 192 224 192S0 159.125 0 118.875V73.125C0 32.875 100.375 0 224 0S448 32.875 448 73.125Z", className: "stria-secondary", style: { opacity: 0.4 } }),
      /* @__PURE__ */ jsx("path", { d: "M224 224.625C136.25 224.625 48.125 209.125 0 176V278.875C0 319.125 100.375 352 224 352S448 319.125 448 278.875V176C399.875 209.125 311.75 224.625 224 224.625ZM0 336V438.875C0 479.125 100.375 512 224 512S448 479.125 448 438.875V336C399.875 369.125 311.75 384.625 224 384.625S48.125 369.125 0 336Z" })
    ]
  }
));
DatabaseDuotone.displayName = "DatabaseDuotone";
var Database_default = DatabaseDuotone;
export {
  Database_default as default
};
