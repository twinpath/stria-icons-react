import { jsx, jsxs } from "react/jsx-runtime";
import React, { forwardRef } from "react";
const MicrowaveDuotone = forwardRef(({ size = 24, color = "currentColor", className, ...props }, ref) => /* @__PURE__ */ jsxs(
  "svg",
  {
    ref,
    xmlns: "http://www.w3.org/2000/svg",
    width: size,
    height: size,
    viewBox: "0 0 576 512",
    fill: color,
    className,
    ...props,
    children: [
      /* @__PURE__ */ jsx("path", { d: "M416 352H96V128H416V352Z", className: "stria-secondary", style: { opacity: 0.4 } }),
      /* @__PURE__ */ jsx("path", { d: "M528 32H48C21.5 32 0 53.5 0 80V400C0 426.5 21.5 448 48 448L72 480H120L144 448H432L456 480H504L528 448C554.5 448 576 426.5 576 400V80C576 53.5 554.5 32 528 32ZM416 352H96V128H416V352ZM496 264C482.75 264 472 253.25 472 240S482.75 216 496 216S520 226.75 520 240S509.25 264 496 264ZM496 168C482.75 168 472 157.25 472 144S482.75 120 496 120S520 130.75 520 144S509.25 168 496 168Z" })
    ]
  }
));
MicrowaveDuotone.displayName = "MicrowaveDuotone";
var Microwave_default = MicrowaveDuotone;
export {
  Microwave_default as default
};
