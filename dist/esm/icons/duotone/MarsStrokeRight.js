import { jsx, jsxs } from "react/jsx-runtime";
import React, { forwardRef } from "react";
const MarsStrokeRightDuotone = forwardRef(({ size = 24, color = "currentColor", className, ...props }, ref) => /* @__PURE__ */ jsxs(
  "svg",
  {
    ref,
    xmlns: "http://www.w3.org/2000/svg",
    width: size,
    height: size,
    viewBox: "0 0 640 512",
    fill: color,
    className,
    ...props,
    children: [
      /* @__PURE__ */ jsx("path", { d: "M272 256C272 291.346 243.346 320 208 320S144 291.346 144 256S172.654 192 208 192S272 220.654 272 256Z", className: "stria-secondary", style: { opacity: 0.4 } }),
      /* @__PURE__ */ jsx("path", { d: "M619.313 244.686L536.971 167.076C521.85 151.957 496.001 162.666 496.001 184.047V223.998L463.999 224V176C464.001 167.162 456.836 160 448.001 160H416C407.161 160 400 167.16 400 176V224H380.954C365.887 142.098 294.258 80 208 80C110.797 80 32 158.797 32 256C32 353.201 110.797 432 208 432C294.258 432 365.889 369.9 380.954 287.998H400V336C400 344.836 407.163 352 416 352H448.001C456.836 352 463.999 344.836 463.999 336V287.998L496.001 288V327.951C496.001 349.334 521.852 360.041 536.971 344.922L619.313 267.312C625.563 261.064 625.561 250.934 619.313 244.686ZM208.002 352C155.065 352 112.002 308.934 112.002 256C112 203.064 155.065 160 208.002 160C260.936 160 304 203.062 304 256C304 308.934 260.936 352 208.002 352Z" })
    ]
  }
));
MarsStrokeRightDuotone.displayName = "MarsStrokeRightDuotone";
var MarsStrokeRight_default = MarsStrokeRightDuotone;
export {
  MarsStrokeRight_default as default
};
