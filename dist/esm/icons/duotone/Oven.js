import { jsx, jsxs } from "react/jsx-runtime";
import React, { forwardRef } from "react";
const OvenDuotone = forwardRef(({ size = 24, color = "currentColor", className, ...props }, ref) => /* @__PURE__ */ jsxs(
  "svg",
  {
    ref,
    xmlns: "http://www.w3.org/2000/svg",
    width: size,
    height: size,
    viewBox: "0 0 448 512",
    fill: color,
    className,
    ...props,
    children: [
      /* @__PURE__ */ jsx("path", { d: "M64 448H384V256H64V448ZM112 288H336C344.838 288 352 295.164 352 304S344.838 320 336 320H112C103.164 320 96 312.836 96 304S103.164 288 112 288ZM0 160V192H448V160H0Z", className: "stria-secondary", style: { opacity: 0.4 } }),
      /* @__PURE__ */ jsx("path", { d: "M0 480C0 497.625 14.375 512 32 512H416C433.625 512 448 497.625 448 480V192H0V480ZM64 256H384V448H64V256ZM336 288H112C103.164 288 96 295.164 96 304S103.164 320 112 320H336C344.838 320 352 312.836 352 304S344.838 288 336 288ZM384 0H64C28.625 0 0 28.625 0 64V160H448V64C448 28.625 419.375 0 384 0ZM80 104C66.75 104 56 93.25 56 80S66.75 56 80 56S104 66.75 104 80S93.25 104 80 104ZM176 104C162.75 104 152 93.25 152 80S162.75 56 176 56S200 66.75 200 80S189.25 104 176 104ZM272 104C258.75 104 248 93.25 248 80S258.75 56 272 56S296 66.75 296 80S285.25 104 272 104ZM368 104C354.75 104 344 93.25 344 80S354.75 56 368 56S392 66.75 392 80S381.25 104 368 104Z" })
    ]
  }
));
OvenDuotone.displayName = "OvenDuotone";
var Oven_default = OvenDuotone;
export {
  Oven_default as default
};
