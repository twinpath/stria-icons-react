import { jsx, jsxs } from "react/jsx-runtime";
import React, { forwardRef } from "react";
const ShovelDuotone = forwardRef(({ size = 24, color = "currentColor", className, ...props }, ref) => /* @__PURE__ */ jsxs(
  "svg",
  {
    ref,
    xmlns: "http://www.w3.org/2000/svg",
    width: size,
    height: size,
    viewBox: "0 0 512 512",
    fill: color,
    className,
    ...props,
    children: [
      /* @__PURE__ */ jsx("path", { d: "M502.258 99.885L411.748 9.377C399.252 -3.119 379 -3.131 366.492 9.377L321.238 54.631C291.777 84.09 285.867 127.943 302.779 163.596L197 269.375L242.256 314.633L348.033 208.854C383.684 225.766 427.543 219.855 457.002 190.395L502.258 145.141C514.764 132.633 514.754 112.381 502.258 99.885ZM411.748 145.141C399.264 157.625 378.967 157.615 366.492 145.141C354.02 132.666 354.008 112.371 366.492 99.885L389.121 77.258L434.375 122.514L411.748 145.141Z", className: "stria-secondary", style: { opacity: 0.4 } }),
      /* @__PURE__ */ jsx("path", { d: "M152.111 224.121C139.611 211.617 119.358 211.617 106.858 224.121L38.973 292.004C-11.033 342.008 -6.283 473.028 16.346 495.657C38.973 518.282 169.988 523.032 219.996 473.028L287.879 405.141C300.381 392.641 300.381 372.391 287.879 359.887L152.111 224.121Z" })
    ]
  }
));
ShovelDuotone.displayName = "ShovelDuotone";
var Shovel_default = ShovelDuotone;
export {
  Shovel_default as default
};
