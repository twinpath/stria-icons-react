import { jsx, jsxs } from "react/jsx-runtime";
import React, { forwardRef } from "react";
const ArrowDownFromDottedLineDuotone = forwardRef(({ size = 24, color = "currentColor", className, ...props }, ref) => /* @__PURE__ */ jsxs(
  "svg",
  {
    ref,
    xmlns: "http://www.w3.org/2000/svg",
    width: size,
    height: size,
    viewBox: "0 0 448 512",
    fill: color,
    className,
    ...props,
    children: [
      /* @__PURE__ */ jsx("path", { d: "M416 95.999C433.674 95.999 448 81.671 448 63.999S433.674 31.999 416 31.999S384 46.327 384 63.999S398.326 95.999 416 95.999ZM320 95.999C337.674 95.999 352 81.671 352 63.999S337.674 31.999 320 31.999S288 46.327 288 63.999S302.326 95.999 320 95.999ZM224 95.999C241.674 95.999 256 81.671 256 63.999S241.674 31.999 224 31.999S192 46.327 192 63.999S206.326 95.999 224 95.999ZM128 95.999C145.674 95.999 160 81.671 160 63.999S145.674 31.999 128 31.999S96 46.327 96 63.999S110.326 95.999 128 95.999ZM32 95.999C49.674 95.999 64 81.671 64 63.999S49.674 31.999 32 31.999S0 46.327 0 63.999S14.326 95.999 32 95.999Z", className: "stria-secondary", style: { opacity: 0.4 } }),
      /* @__PURE__ */ jsx("path", { d: "M73.375 297.374C79.625 291.124 87.812 287.999 96 287.999S112.375 291.124 118.625 297.374L192 370.749V191.999C192 174.312 206.312 159.999 224 159.999S256 174.312 256 191.999V370.749L329.375 297.374C341.875 284.874 362.125 284.874 374.625 297.374S387.125 330.124 374.625 342.624L246.625 470.624C234.125 483.124 213.875 483.124 201.375 470.624L73.375 342.624C60.875 330.124 60.875 309.874 73.375 297.374Z" })
    ]
  }
));
ArrowDownFromDottedLineDuotone.displayName = "ArrowDownFromDottedLineDuotone";
var ArrowDownFromDottedLine_default = ArrowDownFromDottedLineDuotone;
export {
  ArrowDownFromDottedLine_default as default
};
