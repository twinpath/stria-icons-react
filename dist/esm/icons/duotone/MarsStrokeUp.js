import { jsx, jsxs } from "react/jsx-runtime";
import React, { forwardRef } from "react";
const MarsStrokeUpDuotone = forwardRef(({ size = 24, color = "currentColor", className, ...props }, ref) => /* @__PURE__ */ jsxs(
  "svg",
  {
    ref,
    xmlns: "http://www.w3.org/2000/svg",
    width: size,
    height: size,
    viewBox: "0 0 384 512",
    fill: color,
    className,
    ...props,
    children: [
      /* @__PURE__ */ jsx("path", { d: "M256 335.999C256 371.345 227.346 399.999 192 399.999S128 371.345 128 335.999S156.654 272 192 272S256 300.653 256 335.999Z", className: "stria-secondary", style: { opacity: 0.4 } }),
      /* @__PURE__ */ jsx("path", { d: "M224 163.048V144.001H248C252.418 144.001 256 140.423 256 136.001V120.001C256 115.583 252.418 112.001 248 112.001H224V96.001H248.627C265.033 96.001 273.25 76.165 261.65 64.564L200.681 3.596C195.886 -1.197 188.113 -1.197 183.318 3.596L122.349 64.564C110.75 76.165 118.967 96.001 135.373 96.001H160V112.001H136C131.582 112.001 128 115.583 128 120.001V136.001C128 140.423 131.582 144.001 136 144.001H160V163.048C75.099 178.665 11.48 255.062 16.252 345.531C21.035 436.221 98.595 510.67 189.406 511.982C287.795 513.396 368 434.061 368 335.999C368 249.742 305.9 178.11 224 163.048ZM192 431.999C139.064 431.999 96 388.936 96 335.999S139.062 240 192 240C244.933 240 288 283.062 288 335.999S244.933 431.999 192 431.999Z" })
    ]
  }
));
MarsStrokeUpDuotone.displayName = "MarsStrokeUpDuotone";
var MarsStrokeUp_default = MarsStrokeUpDuotone;
export {
  MarsStrokeUp_default as default
};
