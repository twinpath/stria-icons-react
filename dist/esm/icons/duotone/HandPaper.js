import { jsx, jsxs } from "react/jsx-runtime";
import React, { forwardRef } from "react";
const HandPaperDuotone = forwardRef(({ size = 24, color = "currentColor", className, ...props }, ref) => /* @__PURE__ */ jsxs(
  "svg",
  {
    ref,
    xmlns: "http://www.w3.org/2000/svg",
    width: size,
    height: size,
    viewBox: "0 0 512 512",
    fill: color,
    className,
    ...props,
    children: [
      /* @__PURE__ */ jsx("path", { d: "M448 96C430.328 96 416 110.328 416 128V240C416 248.836 408.836 256 400 256S384 248.836 384 240V64C384 46.328 369.672 32 352 32S320 46.328 320 64V240C320 248.836 312.836 256 304 256S288 248.836 288 240V32C288 14.328 273.672 0 256 0S224 14.328 224 32V240C224 248.836 216.836 256 208 256S192 248.836 192 240V64C192 46.328 177.672 32 160 32S128 46.328 128 64V343.438L68.281 283.719C60.469 275.906 50.234 272 40 272C17.151 272 0 290.683 0 312C0 322.234 3.906 332.469 11.719 340.281L124.859 453.422C162.625 491.203 212.859 512 266.281 512H304C401.047 512 480 433.047 480 336V128C480 110.328 465.672 96 448 96ZM368 352H284.801C307.199 374.4 320 403.199 320 432C320 441.6 313.6 448 304 448S288 441.6 288 432C288 387.199 252.801 352 208 352C198.4 352 192 345.6 192 336S198.4 320 208 320H368C377.6 320 384 326.4 384 336S377.6 352 368 352Z", className: "stria-secondary", style: { opacity: 0.4 } }),
      /* @__PURE__ */ jsx("path", { d: "M304 448C294.4 448 288 441.6 288 432L288 432C288 387.2 252.8 352 208 352C198.4 352 192 345.6 192 336S198.4 320 208 320H368C377.6 320 384 326.4 384 336S377.6 352 368 352H284.8C307.2 374.4 320 403.2 320 432C320 441.6 313.6 448 304 448L304 448Z" })
    ]
  }
));
HandPaperDuotone.displayName = "HandPaperDuotone";
var HandPaper_default = HandPaperDuotone;
export {
  HandPaper_default as default
};
