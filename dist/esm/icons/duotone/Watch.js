import { jsx, jsxs } from "react/jsx-runtime";
import React, { forwardRef } from "react";
const WatchDuotone = forwardRef(({ size = 24, color = "currentColor", className, ...props }, ref) => /* @__PURE__ */ jsxs(
  "svg",
  {
    ref,
    xmlns: "http://www.w3.org/2000/svg",
    width: size,
    height: size,
    viewBox: "0 0 384 512",
    fill: color,
    className,
    ...props,
    children: [
      /* @__PURE__ */ jsx("path", { d: "M192 64C85.961 64 0 149.961 0 256S85.961 448 192 448S384 362.039 384 256S298.039 64 192 64ZM192 400C112.598 400 48 335.402 48 256S112.598 112 192 112S336 176.598 336 256S271.402 400 192 400Z", className: "stria-secondary", style: { opacity: 0.4 } }),
      /* @__PURE__ */ jsx("path", { d: "M64 398.82V464C64 490.398 85.6 512 112 512H272C298.4 512 320 490.398 320 464V398.82C286.014 429.301 241.244 448 192 448C142.754 448 97.984 429.301 64 398.82ZM272 0H112C85.6 0 64 21.602 64 48V113.18C97.984 82.695 142.754 64 192 64C241.244 64 286.014 82.699 320 113.18V48C320 21.602 298.4 0 272 0ZM192 152C178.75 152 168 162.75 168 176V248C168 255.125 171.156 261.875 176.625 266.438L224.625 306.438C229.125 310.188 234.562 312 240 312C246.875 312 253.688 309.062 258.438 303.375C266.938 293.188 265.562 278.063 255.375 269.563L216 236.75V176C216 162.75 205.25 152 192 152Z" })
    ]
  }
));
WatchDuotone.displayName = "WatchDuotone";
var Watch_default = WatchDuotone;
export {
  Watch_default as default
};
