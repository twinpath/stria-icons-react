import { jsx } from "react/jsx-runtime";
import React, { forwardRef } from "react";
const GreaterThanEqualThin = forwardRef(({ size = 24, color = "currentColor", className, ...props }, ref) => /* @__PURE__ */ jsx(
  "svg",
  {
    ref,
    xmlns: "http://www.w3.org/2000/svg",
    width: size,
    height: size,
    viewBox: "0 0 448 512",
    fill: color,
    className,
    ...props,
    children: /* @__PURE__ */ jsx("path", { d: "M440 439.998H8C3.594 439.998 0 443.592 0 447.999S3.594 456 8 456H440C444.406 456 448 452.406 448 447.999S444.406 439.998 440 439.998ZM56.562 322.952C57.812 326.078 60.812 327.984 64 327.984C65 327.984 65.999 327.797 66.969 327.422L386.969 199.406C389.999 198.187 392 195.249 392 191.968C392 188.686 389.999 185.748 386.969 184.529L66.969 56.514C62.938 55.013 58.219 56.889 56.562 60.983C54.938 65.077 56.938 69.734 61.031 71.39L362.469 191.968L61.031 312.545C56.938 314.201 54.938 318.858 56.562 322.952Z" })
  }
));
GreaterThanEqualThin.displayName = "GreaterThanEqualThin";
var GreaterThanEqual_default = GreaterThanEqualThin;
export {
  GreaterThanEqual_default as default
};
