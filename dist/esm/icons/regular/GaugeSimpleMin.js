import { jsx } from "react/jsx-runtime";
import React, { forwardRef } from "react";
const GaugeSimpleMinRegular = forwardRef(({ size = 24, color = "currentColor", className, ...props }, ref) => /* @__PURE__ */ jsx(
  "svg",
  {
    ref,
    xmlns: "http://www.w3.org/2000/svg",
    width: size,
    height: size,
    viewBox: "0 0 576 512",
    fill: color,
    className,
    ...props,
    children: /* @__PURE__ */ jsx("path", { d: "M288 32C129 32 0 161 0 320C0 372.75 14.25 422.25 39 464.75C44.625 474.375 55.375 480 66.5 480H509.5C520.625 480 531.375 474.375 537 464.75C561.75 422.25 576 372.75 576 320C576 161 447 32 288 32ZM288 432C265.945 432 248 414.057 248 392S265.945 352 288 352C310.057 352 328 369.943 328 392S310.057 432 288 432ZM500.25 432H365.965C372.166 419.949 376 406.486 376 392C376 343.398 336.602 304 288 304C263.203 304 240.895 314.336 224.898 330.836L129.469 289.938C117.219 284.703 103.156 290.359 97.937 302.547S98.375 328.844 110.531 334.063L202.086 373.301C200.777 379.34 200 385.57 200 392C200 406.486 203.834 419.949 210.035 432H75.75C57.5 397.625 48 359.125 48 320C48 187.625 155.625 80 288 80S528 187.625 528 320C528 359.125 518.5 397.625 500.25 432Z" })
  }
));
GaugeSimpleMinRegular.displayName = "GaugeSimpleMinRegular";
var GaugeSimpleMin_default = GaugeSimpleMinRegular;
export {
  GaugeSimpleMin_default as default
};
