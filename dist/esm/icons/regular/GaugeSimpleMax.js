import { jsx } from "react/jsx-runtime";
import React, { forwardRef } from "react";
const GaugeSimpleMaxRegular = forwardRef(({ size = 24, color = "currentColor", className, ...props }, ref) => /* @__PURE__ */ jsx(
  "svg",
  {
    ref,
    xmlns: "http://www.w3.org/2000/svg",
    width: size,
    height: size,
    viewBox: "0 0 576 512",
    fill: color,
    className,
    ...props,
    children: /* @__PURE__ */ jsx("path", { d: "M288 32C129 32 0 161 0 320C0 372.75 14.25 422.25 39 464.75C44.625 474.375 55.375 480 66.5 480H509.5C520.625 480 531.375 474.375 537 464.75C561.75 422.25 576 372.75 576 320C576 161 447 32 288 32ZM288 432C265.945 432 248 414.057 248 392S265.945 352 288 352C310.057 352 328 369.943 328 392S310.057 432 288 432ZM500.25 432H365.965C372.166 419.949 376 406.486 376 392C376 385.57 375.223 379.34 373.914 373.301L465.469 334.062C477.625 328.844 483.281 314.734 478.063 302.547C472.844 290.344 458.75 284.734 446.531 289.937L351.104 330.836C335.107 314.336 312.797 304 288 304C239.4 304 200 343.398 200 392C200 406.486 203.834 419.949 210.035 432H75.75C57.5 397.625 48 359.125 48 320C48 187.625 155.625 80 288 80S528 187.625 528 320C528 359.125 518.5 397.625 500.25 432Z" })
  }
));
GaugeSimpleMaxRegular.displayName = "GaugeSimpleMaxRegular";
var GaugeSimpleMax_default = GaugeSimpleMaxRegular;
export {
  GaugeSimpleMax_default as default
};
