import { jsx } from "react/jsx-runtime";
import React, { forwardRef } from "react";
const MessageMinusRegular = forwardRef(({ size = 24, color = "currentColor", className, ...props }, ref) => /* @__PURE__ */ jsx(
  "svg",
  {
    ref,
    xmlns: "http://www.w3.org/2000/svg",
    width: size,
    height: size,
    viewBox: "0 0 512 512",
    fill: color,
    className,
    ...props,
    children: /* @__PURE__ */ jsx("path", { d: "M448 0H64C28.75 0 0 28.75 0 64V352.002C0 387.252 28.75 416.002 64 416.002H160V500.002C160 509.838 171.023 515.479 179.125 509.627L304 416.002H448C483.25 416.002 512 387.252 512 352.002V64C512 28.75 483.25 0 448 0ZM464 352.002C464 360.752 456.75 368.002 448 368.002H288L208 428.002V368.002H64C55.25 368.002 48 360.752 48 352.002V64C48 55.25 55.25 48 64 48H448C456.75 48 464 55.25 464 64V352.002ZM336 184.002H176C162.738 184.002 152 194.742 152 208C152 221.256 162.73 231.998 176 231.998H336C349.262 231.998 360 221.258 360 208S349.262 184.002 336 184.002Z" })
  }
));
MessageMinusRegular.displayName = "MessageMinusRegular";
var MessageMinus_default = MessageMinusRegular;
export {
  MessageMinus_default as default
};
